...
class MCStatsTable {
    constructor(tableSelector, statsInstance, playerInstance){
        ...
        this.table.insertAdjacentHTML("beforeend", "".concat(`<mc-header>`, `<h2>Player</h2>`, `<div class="select-wrapper">`, `<select onfocus='this.size=this.children.length;' onblur='this.size=0;' onchange='this.size=1;this.blur();'>`, ...[
            ...this.statsOptions.keys()
        ].map((v)=>`<option>${v}</option>`
        ), `</select>`, `</div>`, `</mc-header>`));
    ...
    /**
   * @private
   * returns {{[key: string]:{valueFunc($_stats:any):number;formatFunc(x:number):string}}} */ prepareOptions() {
        /** @type {Map<string,{valueFunc($_statsInstance:StatsAPI,$_uuid:string):number;formatFunc(x:number):string}>} */ const statsMap = new Map();
        for (const statOption of this.table.children){
            ...
            for (const match of valueExpression.matchAll(namespacePattern)){
                ...
                const matchGroups = Object.entries(match.groups).filter((g)=>g[1] !== undefined
                ).map((g)=>g[0]
                );
                if (matchGroups.includes("namespace")) valueFuncBody += `$_statsInstance.getStatValue("${matchText}",$_uuid)`;
                else valueFuncBody += matchText;
            }
    ...
    generateTable(statOptionKey = [
        ...this.statsOptions.entries()
    ][0][0]) {
        const statOption = this.statsOptions.get(statOptionKey);
        this.rowDiv.innerHTML = "";
        /** @type [number,HTMLElement][] */ const statTableList = [];
        ...
        statTableList.sort((a, b)=>b[0] - a[0]
        ).forEach((e)=>{
